...
export async function POST(request: Request) {
  ...
  try {
    ...
    lead = validateLead(raw.lead);
    result = calculateIht(raw.calculator);
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : 'Check your details.' }, 400);
  }
  try {
    if (!(await allowRequest(request)))
      return json({ error: 'Please wait 15 minutes before requesting another report.' }, 429);
    const record = await saveLead(lead.requestId, 'iht-report', {
      lead,
    ...
    const delivery =
      record.emailStatus === 'sent'
        ...
        : await emailMessage(
            lead.requestId,
            lead.email,
            'Your requested inheritance-tax illustration',
            ...
            { filename: 'better-call-sim-iht-report.pdf', content: pdf },
          );
    await updateDelivery(lead.requestId, delivery);
    const callbackDelivery = !lead.callback
      ? 'not-requested'
      : record.callbackStatus === 'sent'
        ? 'sent'
        : await emailMessage(
            lead.requestId + '-callback',
            runtime().ENQUIRY_TO_EMAIL ?? '',
            'IHT report callback requested',
    ...
    return json(
      {
        error: 'We could not generate your report. Your answers are still here; please try again.',
      },
      503,
    );
  }
